const fetchPokemon = () => {
  ...
  const url = `http://pokeapi.co/api/v2/pokemon/${pokeName}`;
...
fetch(url).then((res) => {
    ...
    if (data) {
        ...
        element4.innerHTML=`Height: ${data.height}`;
        ...
        element8.innerHTML=`Move 2: ${data.movies[1].move.name}`;

        let element9=document.getElementById('pokemove3')
        element9.innerHTML=`Move 3: ${data.movies[2].move.name}`;

        let element10=document.getElementById('pokemove4')
        element10.innerHTML=`Move 4: ${data.movies[3].move.name}`;
